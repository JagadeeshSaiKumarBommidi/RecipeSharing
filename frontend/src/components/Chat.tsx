  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20 md:pb-6">
      <div className="max-w-6xl mx-auto p-4 sm:p-6">
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden h-[calc(100vh-8rem)] sm:h-[600px]">
          <div className="flex h-full">
            {/* Friends List */}
            <div className={`${selectedFriend ? 'hidden md:block' : 'block'} w-full md:w-1/3 border-r border-gray-200`}>
              <div className="p-4 border-b border-gray-200">
                <h2 className="text-lg sm:text-xl font-bold text-gray-900 mb-4">Messages</h2>
                <div className="relative">
                  <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 w-5 h-5" />
                  <input
                    type="text"
                    placeholder="Search conversations..."
                    className="w-full pl-10 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-all"
                  />
                </div>
              </div>
              ...
              <div className="overflow-y-auto h-full">
                {friends.length === 0 ? (
                  <div className="text-center py-12 px-4">
                    ...
                    <p className="text-sm sm:text-base text-gray-600">Add friends to start chatting!</p>
                  </div>
                ) : (
                  <div className="p-2">
                    {friends.map((friend) => (
                      <button
                        ...
                        className={`w-full flex items-center space-x-3 p-3 rounded-xl hover:bg-gray-50 transition-colors ${
                          selectedFriend?._id === friend._id ? 'bg-orange-50 border-l-4 border-orange-500' : ''
                        }`}
                      >
              ...
              {selectedFriend ? (
                <>
                  ...
                  <div className="p-4 border-b border-gray-200 flex items-center">
                    ...
                    <div>
                      <h3 className="font-semibold text-gray-900 text-sm sm:text-base">{selectedFriend.fullName}</h3>
                      <p className="text-xs sm:text-sm text-gray-500">@{selectedFriend.username}</p>
                    </div>
                  </div>
                  ...
                  <div className="p-4 border-t border-gray-200">
                    <div className="flex space-x-3">
                      <input
                        ...
                        className="flex-1 px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-2xl focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-all text-sm sm:text-base"
                        onKeyPress={(e) => e.key === 'Enter' && sendMessage()}
                      />
                      <button
                        onClick={sendMessage}
                        disabled={!newMessage.trim()}
                        className="p-2.5 bg-gradient-to-r from-orange-500 to-red-500 text-white rounded-2xl hover:from-orange-600 hover:to-red-600 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                      >
